    ...
    for info in file_info:
        if detailed:
            try:
                ...
                print(f"{info['name'][:20]:<20} {format_file_size(info['size']):<8} {info['modified'].strftime('%Y-%m-%d %H:%M')}{data_info}")
            except Exception as e:
                print(f"{info['name'][:20]:<20} {format_file_size(info['size']):<8} {info['modified'].strftime('%Y-%m-%d %H:%M')} - Error reading: {e}")
        else:
...
def main():
    ...
    clear_old_parser.add_argument('--hours', type=int, default=24, help='Clear files older than N hours')
