...
from database.models import (
    PositionSide, TradeSource, OrderStatus, EventType,
    Trade, Position, AccountHistory, PerformanceMetrics,
    TradingSession, SystemEvent, StrategyExecution,
    AccountBalance, OptimizationCycle, PredictionPerformance
)
...
class DummyDataPopulator:
    ...
    def generate_trade_data(self, trade_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'take_profit': entry_price * (1.05 if side == PositionSide.LONG else 0.95),
            'strategy_config': {
    ...
    def create_trading_sessions(self) -> List[int]:
    ...
    def populate_trades(self, session_ids: List[int], num_trades: int = 100):
    ...
    def populate_positions(self, session_ids: List[int], num_positions: int = 15):
    ...
    def populate_account_history(self, session_ids: List[int], num_snapshots: int = 50):
        ...
        for session_id in session_ids:
            ...
            for i in range(num_snapshots // len(session_ids)):
    ...
    def populate_system_events(self, session_ids: List[int], num_events: int = 30):
        """Populate system events table with dummy data."""
        logger.info(f"Creating {num_events} system events...")
        
        event_types = list(EventType)
        components = ['trading_engine', 'strategy_manager', 'risk_manager', 'data_provider', 'ml_model']
        ...
        for i in range(num_events):
            ...
            if event_type == EventType.ENGINE_START:
                ...
                message = f"ML model updated with new training data"
    ...
    def populate_strategy_executions(self, session_ids: List[int], num_executions: int = 80):
        ...
        for i in range(num_executions):
            ...
            self.db_manager.log_strategy_execution(
                ...
                reasons=[random.choice(['Strong signal', 'Risk management', 'Market conditions', 'ML prediction'])],
    ...
    def populate_account_balances(self, session_ids: List[int]):
            ...
            for i in range(5):
                ...
                self.db_manager.update_balance(
                    ...
                    update_reason=random.choice([
                        "Trade P&L", "Manual adjustment", "Deposit", "Withdrawal", "Fee adjustment"
                    ]),
    ...
    def populate_optimization_cycles(self, session_ids: List[int], num_cycles: int = 10):
    ...
    def populate_prediction_performance(self, session_ids: List[int], num_records: int = 20):
    ...
    def end_sessions(self, session_ids: List[int]):
...
def main():
    ...
    parser.add_argument('--trades', type=int, default=100, help='Number of trades to create (default: 100)')
    parser.add_argument('--database-url', type=str, help='Database URL (optional, uses environment variable if not provided)')
    parser.add_argument('--confirm', action='store_true', help='Skip confirmation prompt')
    
    args = parser.parse_args()
    
    if not args.confirm:
        response = input(f"This will populate the database with {args.trades} trades and related data. Continue? (y/N): ")
        if response.lower() != 'y':
