    ...
    try:
        ...
        try:
            ...
            if len(input_info.shape) >= 3:
                ...
                dummy_input = np.random.random((batch_size, sequence_length, feature_count)).astype(
                    np.float32
                )
                ...
                try:
                    ...
                    results["info"]["output_value_range"] = [
                        float(np.min(output[0])),
                        float(np.max(output[0])),
                    ]
            ...
            results["warnings"].append(
                "ONNX/ONNXRuntime not available - skipping model loading test"
            )
        except Exception as e:
            results["valid"] = False
        ...
        if expected_symbol:
            ...
            if expected_filename_part not in model_filename:
                results["warnings"].append(
                    f"Model filename doesn't contain expected symbol: {expected_symbol}"
                )
