...
data["close_normalized"] = (
    ...
    .apply(lambda x: (x[-1] - np.min(x)) / (np.max(x) - np.min(x)), raw=True)
)
...
while current_date < end_date:
    ...
    predictions_list.append(
        {"date": current_date, "prediction": prediction, "sentiment": sentiment}
    )
...
comparison_df = pd.merge(
    predictions_df, data[["timestamp", "close"]], left_on="date", right_on="timestamp"
)
...
investment_df["price_direction"] = np.where(
    investment_df["prediction"].shift(-1) > investment_df["prediction"], 1, -1
)
investment_df["sentiment_direction"] = np.where(investment_df["sentiment"] > 0, 1, -1)
investment_df["position"] = np.where(
    investment_df["price_direction"] == investment_df["sentiment_direction"],
    investment_df["price_direction"],
    0,
)
investment_df["strategy_returns"] = (
    investment_df["position"]
    * (investment_df["actual"].shift(-1) - investment_df["actual"])
    / investment_df["actual"]
)
investment_df["buy_and_hold_returns"] = (
    investment_df["actual"].shift(-1) - investment_df["actual"]
) / investment_df["actual"]
...
investment_df["drawdown"] = (investment_df["strategy_returns"] - investment_df["peak"]) / (
    1 + investment_df["peak"]
)
...
plt.plot(
    ...
    label="Buy and Hold Cumulative Returns",
    color="orange",
)
plt.xlabel("Date")
...
for train_index, test_index in tscv.split(data):
    ...
    train.loc[:, "close_normalized"] = (
        train["close"]
        .rolling(window=sequence_length, min_periods=1)
        .apply(lambda x: (x[-1] - np.min(x)) / (np.max(x) - np.min(x)), raw=True)
    )
    test.loc[:, "close_normalized"] = (
        test["close"]
        .rolling(window=sequence_length, min_periods=1)
        .apply(lambda x: (x[-1] - np.min(x)) / (np.max(x) - np.min(x)), raw=True)
    )
...
ax1.plot(
    comparison_df["date"], comparison_df["prediction"], label="Predicted Price", color="tab:orange"
)
...
plt.show()
print(
    f"Graph of price, prediction, and sentiment saved as '{symbol.replace('/', '_')}_price_prediction_sentiment.png'"
)
