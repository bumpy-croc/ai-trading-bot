...
def create_features(df, time_steps=120):
    ...
    # Sentiment features are already normalized to [-1, 1], but let's standardize them
...
def create_model_with_sentiment(input_shape, num_features):
    """
    Create a more sophisticated model that can handle sentiment features
    """
    # Input layer
...
def main():
    """Main training function with sentiment integration"""
    parser = argparse.ArgumentParser(description='Train a neural network model with sentiment data for cryptocurrency price prediction')
    ...
    print(f"🚀 Starting model training for {args.symbol} with sentiment analysis")
    ...
    try:
        ...
        if not args.no_sentiment:
            ...
            if not sentiment_df.empty:
                print(f"Loaded {len(sentiment_df)} sentiment data points")
                
                # Resample sentiment data to match price timeframe
                if args.timeframe != '1d':
                    sentiment_df = sentiment_provider.resample_to_timeframe(args.timeframe)
            else:
        ...
        if not args.no_sentiment and len(feature_names) > 5:
            model = create_model_with_sentiment(
                input_shape=(X_train.shape[1], X_train.shape[2]),
                num_features=len(feature_names)
            )
            model_suffix = "sentiment"
        else:
        ...
        if not args.no_sentiment:
            print(f"🎯 Sentiment features included: {len([f for f in feature_names if f.startswith('sentiment_')])}")
