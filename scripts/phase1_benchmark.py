...
def run_training(
    symbol: str, model_type: str, days: int, epochs: int, variant: str = "default"
...
def load_model_metadata(symbol: str, model_type: str) -> dict:
    ...
    metadata_path = (
        project_root / "src" / "ml" / "models" / symbol / "price" / "latest" / "metadata.json"
    )
...
def quick_validation(symbol: str = "BTCUSDT"):
    ...
    for model_type, variant, name in models_to_test:
        ...
        success, duration, metrics = run_training(
            symbol, model_type, days=7, epochs=5, variant=variant
        )
...
def full_benchmark(symbol: str = "BTCUSDT"):
    ...
    for model_type, variant, name in models_to_test:
        ...
        success, duration, metrics = run_training(
            symbol, model_type, days=30, epochs=20, variant=variant
        )
...
def analyze_results(results: list[dict]):
    ...
    print(
        f"{'Model':<20} {'RMSE Change':<15} {'MAPE Change':<15} {'Time Ratio':<15} {'Status':<10}"
    )
    print("-" * 75)
    ...
    best_accuracy = min(
        (r for r in results if r["success"] and "rmse" in r["metrics"]),
        key=lambda r: r["metrics"]["rmse"],
        default=None,
    )

    fastest = min((r for r in results if r["success"]), key=lambda r: r["duration"], default=None)

    if best_accuracy:
        rmse_improvement = (
            (baseline_rmse - best_accuracy["metrics"]["rmse"]) / baseline_rmse
        ) * 100
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run quick validation only (7 days, 5 epochs)"
    )
    parser.add_argument(
        "--full", action="store_true", help="Run full benchmark (30 days, 20 epochs)"
    )
    parser.add_argument(
        "--analyze-only", action="store_true", help="Analyze existing models without training"
    )
