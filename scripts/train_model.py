...
def assess_sentiment_data_quality(sentiment_df: pd.DataFrame, price_df: pd.DataFrame) -> dict:
    ...
    if missing_dates:
        ...
        assessment['missing_periods'] = [(start, end) for start, end in gap_starts 
                                       if (end - start).days > 7]
    ...
    assessment['quality_score'] = (coverage_score * 0.5 + freshness_score * 0.3 + completeness_score * 0.2)
...
def validate_model_robustness(model, X_test, y_test, feature_names, has_sentiment=True):
    ...
    if has_sentiment:
        ...
        if sentiment_indices:
            ...
            print(f"   Performance degradation without sentiment: {results['no_sentiment_performance']['degradation_pct']:.1f}%")
...
def main():
    ...
    parser.add_argument('--force-price-only', action='store_true',
    ...
    try:
        ...
        if sentiment_assessment['recommendation'] in ['full_sentiment', 'hybrid_with_fallback']:
            ...
            print(f"Using price-only dataset")
        ...
        has_sentiment = sentiment_assessment['recommendation'] in ['full_sentiment', 'hybrid_with_fallback']
        ...
        create_training_plots(history, model, X_test, y_test, feature_names, args.symbol, model_type, output_dir, has_sentiment)
        
        # Evaluate model performance
        print(f"\n📈 Evaluating model performance...")
        evaluation_results = evaluate_model_performance(model, X_train, y_train, X_test, y_test, scalers.get('close'))
        ...
        if has_sentiment and 'no_sentiment_performance' in robustness_results:
            ...
            if deg < 20:
                print(f"🎯 Excellent robustness: Only {deg:.1f}% performance loss without sentiment")
            elif deg < 50:
...
def create_training_plots(history, model, X_test, y_test, feature_names, symbol, model_type, output_dir, has_sentiment):
    ...
    try:
        ...
        plt.plot(test_predictions[:sample_size].flatten(), label='Predicted', alpha=0.8, linewidth=2)
