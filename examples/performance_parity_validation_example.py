...
def create_sample_backtest_results(
    ...
    return pd.DataFrame({"timestamp": dates, "balance": balance, "trade_pnl": trade_pnl})
...
def example_basic_validation():
    ...
    legacy_results = create_sample_backtest_results(
        daily_return_mean=0.001, daily_return_std=0.02, seed=42
    )

    new_results = create_sample_backtest_results(
        daily_return_mean=0.0012, daily_return_std=0.021, seed=43  # Slightly different performance
    )
    ...
    for comparison in report.metric_comparisons:
        ...
        print(
            f"{status_symbol} {comparison.metric_name}: "
...
def example_export_functionality():
    ...
    validator = PerformanceParityValidator()
    report = validator.validate_performance_parity(
        legacy_results, new_results, "Export Example Strategy", "legacy", "new"
    )
