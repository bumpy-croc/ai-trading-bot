...
def main():
    ...
    signal_generators = [
        ...
        WeightedVotingSignalGenerator({
            HoldSignalGenerator(): 0.3,
            RandomSignalGenerator(seed=42): 0.7
        })
    ]
    ...
    risk_managers = [
        FixedRiskManager(risk_per_trade=0.02),
        VolatilityRiskManager(base_risk=0.02)
    ]
    ...
    regime_test = regime_tester.test_component_in_regime(
        best_signal_generator, 
        list(regime_stats.keys())[0]  # Test in first regime
    )
    ...
    replacement_impact = attribution_analyzer.analyze_component_replacement_impact(
        None,  # Would need actual strategy object
        'signal',
        RandomSignalGenerator(seed=42)
    )
    ...
    print(f"🏆 Best Risk Manager: {best_risk[0]} (Efficiency: {best_risk[1].risk_efficiency_score:.3f})")
    print(f"🏆 Best Position Sizer: {best_sizing[0]} (Optimality: {best_sizing[1].optimal_sizing_score:.3f})")
