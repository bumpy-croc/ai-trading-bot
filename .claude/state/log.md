...
## 2026-07-07 07:05 · decision · daemon(PM) · [D-2026-07-07-02]
**#925 (cloud-first training pipeline) merged to develop after one-reviewer gauntlet + fix round; closes #918 and #928.**
- Review verdict was REQUEST-CHANGES on one P1: `--model-type tft` (newly enabled) crashed at model construction on the has_sentiment kwarg — the same defect class the architecture tournament hit live on attention_lstm/default (#928). Two independent verifications (tournament-worktree patch + reviewer repro) converged on the identical fix: `kwargs.pop("has_sentiment", True)` in create_model before dispatch. Fix round added an 18-pair (model_type, variant) construction smoke test with the trainer's exact kwargs, TDD red-run verified.
- Ships: cache-first training ingestion (loud fail, no third-party fallback for corpora, #909), force_price_only→PriceOnlyFeatureExtractor with close_normalized target (raw-close target bug fixed upstream), mixed-timestamp datetime hardening, --model-type/--model-variant threading through cloud training, cloud data channel unified with local corpus loader.
- **Queued follow-up: ECR training image rebuild required before any cloud training job** (weekly retrain Sunday has an image-freshness precondition that enforces this; target-redesign tournament also blocked on it). Deliberately deferred while entrant 2 trains locally to avoid compute contention.
Ref: GH #925, #918, #928, #909 · review at scratchpad/review_925_code.md
...
## 2026-07-08 00:15 · decision · daemon(PM) · [D-2026-07-08-01]
...
## 2026-07-10 13:56 · track-record · ml-engineer
TARGET-REDESIGN tournament Phase 2 scaffolding (GH #933) · event: engineering-build (no training runs)
Built the 5-item engineering-work inventory from the LOCKED preregistration doc (docs/research/experiments/2026-07-10_target-redesign-tournament-prereg.md §8), on branch feat/target-tournament-scaffolding in worktree .claude/worktrees/target-tournament-scaffolding (never touched the PM worktree): (1) alt-target label generation (binary_direction/triple_barrier/smoothed_return, src/ml/training_pipeline/labels.py) built on a new shared src/engines/shared/barrier_touch.py extracted from ExitHandler's SL/TP intrabar logic — behavior-preserving refactor, full tests/unit/backtesting/ suite green (134 passed); (2) the #947 guard (task_types.py, validate_target_head_compatibility, closes #947 — tft's binary head previously trained silently against the regression target) plus a real-compiled-head-vs-declared-task-type ground-truth test extending the #937 15-pair smoke matrix; (3) classification-native prediction/signal path — ModelPrediction/PredictionResult gain a purely-additive `probabilities` field, OnnxRunner interprets metadata task_type+class_labels, new src/prediction/distribution_stats.py (FrozenDistribution/percentile_rank_confidence) is the harness-wide fix for the prohibited confidence=|x|×12 formula, new src/strategies/components/exam_signal_generator.py consumes it (MLBasicSignalGenerator/MLSignalGenerator untouched); (4) meta-labeling entrant scaffolding (src/ml/training_pipeline/meta_labels.py: primary-signal fire-point runner, exam-geometry trade-profitability simulator, §2a feature set with EnhancedRegimeDetector reuse); (5) exam-only strategy factory (src/strategies/exam_target_redesign.py, mirrors ml_basic's CoreRiskAdapter+ConfidenceWeightedSizer(0.2,0.3) wiring, NOT HyperGrowth's FlatRiskManager — ratified risk-limits.json stop/tp/max-holding defaults).
Metrics: TDD throughout, hand-computable fixtures for triple-barrier + meta-label profitability simulation, explicit leak-boundary tests. atb dev quality --changed clean (black/ruff/mypy), bandit zero findings. Full unit suite 5026 passed, 1 skipped (pre-existing, unrelated).
Ref: PR #948 (open, not merged — awaiting the two-reviewer architecture/code-review gauntlet the PM will run given this touches money-path-adjacent code: src/prediction/engine.py, src/prediction/models/onnx_runner.py, src/engines/backtest/execution/exit_handler.py), issue #947 (closes), issue #933 (Phase 2 of).

## 2026-07-12 09:05 · track-record · quant-researcher
Experiment (Lane D, trade-review skill): live ETHUSDT trade autopsy 2026-06-02→2026-07-12 (12 closed trades + open #22) → inconclusive on strategy edge (n too small), 4 hypotheses handed to experiment-preregister
Evidence: docs/research/notes/2026-07-12_live-trade-review.md (PR #960, issue #961, worktree .claude/worktrees/live-trade-review-0712, branch docs/live-trade-review-2026-07-12). MFE/MAE reconstructed from cached OHLCV (independent of trades.mfe/mae — see below): win-streak (6 trades) averaged ~72% capture of peak favorable excursion; the one big loser (trade #5, -10.00%) built near-zero MFE (0.99%) before riding ~91% of its MAE to the wide stop. Exit-reason ledger decomposition: ops-failure/emergency closes net ~-$0.06 (noise, correctly separated from strategy signal per skill); six win-streak "Stop loss" exits net +$1.74 (ledger-verified, the entire positive story). Position #22 (open 9.8d, entry confidence 0.25): MFE +2.13%/MAE -8.05% against the 10%/30% wide-stop design — 81% of stop budget used, never past 7% of TP distance. Matched-config backtest (post-#838 engine, identical window, fees+slippage on) diverged sharply: 6 trades/-0.78% vs live's 12 trades/≈+9% — well outside the 15% parity KPI, consistent with (not newly diagnosing) the existing forming-bar-fliprate and confidence-collapse findings.
Two data-integrity findings surfaced, documented not fixed (out of scope, no code touched): (1) trades.mfe/mae columns disagree with their own mfe_price/mae_price companions by a non-constant ~10-23x across trades — flagged in issue #961 for follow-up, not a new GH issue per the task's small-scope instruction. (2) trade #5's -10.00% loss has no isolated ledger row near its exit; coincides with the already-documented 2026-06-05 margin_equity_sync_correction (not a new incident).

## 2026-07-12 10:15 · track-record · quant-researcher
Experiment (Lane C): staging bear-market flag cohort verdicts (2026-07-06→07-12, regime detection / exposure governor / macro event guard / vol-target sizing / circuit breakers dry_run / FEATURE_ALLOW_CROSS_SYMBOL_MODEL) → EXTEND all, no promotions this cycle
Evidence: docs/research/ops-snapshots/2026-07-12_staging-cohort-verdicts.md (PR #963, issue #964). Read-only Postgres against staging+prod public proxy (no mutations, no railway command outside LESSONS §3 safe list). Ground-truth trade/position counts used throughout (not raw strategy_executions.action_taken tallies, which log a decision every ~30-60s and overstate real entries ~300x on both staging AND prod — a system-wide artifact, not cohort-specific). ~20 stale OPEN position rows from dead sessions (pre-dating cohort by weeks) identified and excluded; session 19→20 boundary's session_id-reassignment-on-still-open-position quirk confirmed and handled via entry_time-based chronology, not session grouping.
Key finding: regime detector never labeled TREND_DOWN once in 8 days live (07-04 23:24→present) — trend_up 84%/range 16%/trend_down 0% — so the entire downstream bear-defense chain (exposure governor's tight caps, vol-target sizing) was never exercised against its actual purpose; the cohort's own premise went untested by market conditions, not by code defect. Exposure governor and cross-symbol-model guard confirmed wired-correctly-and-inert (zero binding events, fully explained by regime mix / native-model availability). Macro event guard never fired because config/macro_events.json has the wrong FOMC July date (07-29 instead of the real 07-08) — filed GH #962, fix landed as PR #965 (fix/macro-events-fomc-date, 3 regression tests added, 244/244 unit tests green, mypy/black/ruff clean) in parallel with this analysis, left open for review, not merged. Circuit-breaker dry_run trips log only to ephemeral container stdout, never to system_events — cannot verify ~98% of the cohort window; flagged as the report's largest instrumentation gap. Vol-target sizing's effect is structurally unobservable post-hoc (returns a float, no tag/log) — real trade sizes (14.4-19.9%) overlap too much with pre-cohort/prod flat-sizing baselines (4.8-20% / 7.2-14.4%) to say anything with n=4.
Staging vs prod (same window, normalized): staging +1.32% net / 0.64% max DD / 3 closed trades all winners; prod -0.17% net / 1.01% max DD (mark-to-market swing on one 10-day-old open short) / 0 new trades — comparison is clean at the code level (develop-parity) but underpowered (n=3 vs n=0) to attribute any outcome to the flags.
kelly_momentum paper trial: NEVER STARTED — zero trading_sessions rows ever; staging has no second vehicle to host it concurrently with HyperGrowth. The 2026-07-04 ops-feasibility check (proposal 2026-07-04-01) was never run; parked 8 days past its own "next week" target. Recommend PM decide: provision or formally close.
No proposal filed — nothing recommended for promotion this cycle, so no board/risk-officer action required yet. risk_review_required=true flagged in the write-up for exposure governor, macro event guard, vol-target sizing, and circuit breakers whenever real bear-regime evidence eventually accumulates.
Ref: PR #963, PR #965 (macro_events.json fix, closes #962), issue #964, docs/research/ops-snapshots/2026-07-12_staging-cohort-verdicts.md
Ref: PR #960, issue #961

## 2026-07-12 · track-record · risk-officer (via pm)
PR #1001 (fix/drawdown-gate-ordering): verdict=SAFE-WITH-CONDITIONS, confidence=high. Closes the one-entry breach-bar leak (fresh entry up to 0.20 notional could open before close-only latched on the 20%-cap bar); worst-case ~2% of balance ×leverage. All 4 exposure-increase chokepoints gated (entry eval, execute_entry_locked, legacy short, scale-in provider); durable-peak throttle fix (reads DB session peak not resettable tracker peak, #845 class). Scenarios checked: phantom-peak false-halt (mitigated—guard excludes tracker from seeding), transient-balance-understatement false close-only (fail-safe, needs >20% glitch), throttle over-throttle-to-zero (impossible, floor 0.4), guard×halt×entry-pause×circuit-breaker interaction (clean), prod session-20 @0.5% DD (no sizing/halt change).
Conditions: (1) GATING—verify live active-session peak ≈$84.44. (2) resolve changelog conflict + re-run suite on merged tree. (3) 24-48h post-deploy watch for spurious close-only.
