...
- **Side-findings → issues + agents dispatched**: #914 `strategy_executions.ml_predictions` is JSON null in every row ever (prod+staging) — fix agent dispatched (separate PR, kept file-disjoint from the #913 fix session). #915 prod `$ALERT_WEBHOOK_URL` unset → `NO_ALERT_CHANNEL`; USER_WS_DEGRADED escalated 3→1360 reconnects 07-04→07-06 with zero operator paging (double-blind alerting recurrence); read-only live-ops verification pass dispatched (post-#911-deploy WS health + webhook config state per env). Commented evidence on #723.
