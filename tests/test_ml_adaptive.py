...
class TestMlAdaptive:
    ...
    def test_market_regime_detection(self, strategy, sample_data):
        ...
        crash_period = df.iloc[
            len(df) // self.CRASH_START_DIVISOR : len(df) // self.CRASH_START_DIVISOR + 96
        ]
    ...
    def test_daily_loss_limit(self, strategy):
        ...
        df = pd.DataFrame(
            {"close": 10000, "open": 10000, "high": 10100, "low": 9900, "volume": 1000}, index=dates
        )
