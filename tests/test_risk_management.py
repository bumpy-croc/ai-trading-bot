...
class TestRiskManager:
    ...
    def test_drawdown_monitoring(self, risk_parameters):
        ...
        # Test no drawdown
        assert risk_manager.check_drawdown(10000, 10000) == False
        
        # Test acceptable drawdown
        assert risk_manager.check_drawdown(9000, 10000) == False  # 10% drawdown
        
        # Test excessive drawdown
        assert risk_manager.check_drawdown(7500, 10000) == True   # 25% drawdown (exceeds 20% limit)
        
        # Test edge case
        peak_balance = 10000
        max_allowed_drawdown = peak_balance * (1 - risk_parameters.max_drawdown)
        assert risk_manager.check_drawdown(max_allowed_drawdown + 1, peak_balance) == False
        assert risk_manager.check_drawdown(max_allowed_drawdown - 1, peak_balance) == True
    ...
    def test_maximum_position_size_enforcement(self, risk_parameters):
        ...
        assert actual_position_value <= max_allowed_position_value + 0.01  # Small tolerance for rounding
...
class TestEdgeCases:
    ...
    def test_very_large_values(self, risk_parameters):
        ...
        # Very large balance
        position_size = risk_manager.calculate_position_size(
            price=50000, atr=1000, balance=1e12, regime='normal'  # $1 trillion
        )
