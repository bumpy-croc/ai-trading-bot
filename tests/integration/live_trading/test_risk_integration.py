...
class TestRiskIntegration:
    ...
    def test_risk_manager_integration(self, mock_strategy, mock_data_provider, risk_parameters):
        ...
        engine = LiveTradingEngine(strategy=mock_strategy, data_provider=mock_data_provider, risk_parameters=risk_parameters, max_position_size=0.05)
    ...
    def test_drawdown_monitoring(self, mock_strategy, mock_data_provider):
        if not LIVE_TRADING_AVAILABLE:
            pytest.skip("Live trading components not available")
        engine = LiveTradingEngine(strategy=mock_strategy, data_provider=mock_data_provider, initial_balance=10000)
    ...
    def test_maximum_positions_limit(self, mock_strategy, mock_data_provider):
        ...
        for i in range(max_positions + 2):
            engine._open_position(symbol=f"COIN{i}USDT", side=PositionSide.LONG, size=0.01, price=1000)
