...
class TestThreadSafety:
    ...
    def test_concurrent_position_updates(self, mock_strategy, mock_data_provider):
        ...
        def open_positions():
            if hasattr(engine, '_open_position'):
                for i in range(3):
                    try:
                        engine._open_position(symbol=f"BTC{i}USDT", side="LONG", size=0.02, price=50000 + i)
                    except Exception:
                        pass
                    time.sleep(0.01)
        def close_positions():
    ...
    def test_stop_event_handling(self, mock_strategy, mock_data_provider):
        if not LIVE_TRADING_AVAILABLE:
            pytest.skip("Live trading components not available")
        engine = LiveTradingEngine(strategy=mock_strategy, data_provider=mock_data_provider, check_interval=1)
        mock_data_provider.get_live_data.return_value = pd.DataFrame({'open': [50000], 'high': [50100], 'low': [49900], 'close': [50050], 'volume': [1000]}, index=[datetime.now()])
        def run_trading():
