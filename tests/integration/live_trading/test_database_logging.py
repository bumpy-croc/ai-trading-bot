...
try:
    ...
    from database.models import Trade, TradeSource, Position, PositionSide, OrderStatus, SystemEvent, EventType, AccountHistory, AccountBalance, PerformanceMetrics, StrategyExecution, TradingSession
    DB_AVAILABLE = True
except Exception:
    DB_AVAILABLE = False
...
class TestDatabaseLogging:
    def test_trades_logged_to_database(self, mock_strategy, mock_data_provider):
        ...
        session_id = db_manager.create_trading_session(
            strategy_name="TestStrategy", symbol="BTCUSDT", timeframe="1h", initial_balance=10000, strategy_config={"test": True}, mode="live"
        )
        trade_data = {
            'symbol': 'BTCUSDT', 'side': PositionSide.LONG, 'entry_price': 50000.0, 'exit_price': 51000.0, 'size': 0.1,
            'entry_time': datetime.now() - timedelta(hours=1), 'exit_time': datetime.now(), 'pnl': 100.0, 'exit_reason': 'take_profit',
            'strategy_name': 'TestStrategy', 'source': TradeSource.PAPER, 'order_id': 'test_order_001', 'session_id': session_id
        }
    ...
    def test_positions_logged_to_database(self, mock_strategy, mock_data_provider):
        ...
        session_id = db_manager.create_trading_session(strategy_name="TestStrategy", symbol="BTCUSDT", timeframe="1h", initial_balance=10000, mode="live")
        position_data = {
            'symbol': 'BTCUSDT', 'side': PositionSide.LONG, 'entry_price': 50000.0, 'size': 0.1, 'strategy_name': 'TestStrategy', 'order_id': 'test_position_001',
            'stop_loss': 49000.0, 'take_profit': 52000.0, 'confidence_score': 0.75, 'quantity': 0.002, 'session_id': session_id
        }
    ...
    def test_account_history_snapshots_logged(self, mock_strategy, mock_data_provider):
        ...
        session_id = db_manager.create_trading_session(strategy_name="TestStrategy", symbol="BTCUSDT", timeframe="1h", initial_balance=10000, mode="live")
        snapshot_data = {
            'balance': 10100.0, 'equity': 10150.0, 'total_pnl': 150.0, 'open_positions': 2, 'total_exposure': 5000.0, 'drawdown': 0.05, 'daily_pnl': 50.0, 'margin_used': 2500.0, 'session_id': session_id
        }
    ...
    def test_account_balance_logged(self, mock_strategy, mock_data_provider):
        db_manager = DatabaseManager()
        session_id = db_manager.create_trading_session(strategy_name="TestStrategy", symbol="BTCUSDT", timeframe="1h", initial_balance=10000, mode="live")
        success = db_manager.update_balance(new_balance=10200.0, update_reason='trade_pnl', updated_by='system', session_id=session_id)
        assert success
    ...
    def test_performance_metrics_logged(self, mock_strategy, mock_data_provider):
        ...
        session_id = db_manager.create_trading_session(strategy_name="TestStrategy", symbol="BTCUSDT", timeframe="1h", initial_balance=10000, mode="live")
        trade_data = {
            'symbol': 'BTCUSDT', 'side': 'LONG', 'entry_price': 50000.0, 'exit_price': 51000.0, 'size': 0.1,
            'entry_time': datetime.now() - timedelta(hours=2), 'exit_time': datetime.now() - timedelta(hours=1), 'pnl': 100.0,
            'exit_reason': 'take_profit', 'strategy_name': 'TestStrategy', 'session_id': session_id
        }
    ...
    def test_strategy_execution_data_logged(self, mock_strategy, mock_data_provider):
        ...
        session_id = db_manager.create_trading_session(strategy_name="TestStrategy", symbol="BTCUSDT", timeframe="1h", initial_balance=10000, mode="live")
        execution_data = {
            'strategy_name': 'TestStrategy', 'symbol': 'BTCUSDT', 'signal_type': 'entry', 'action_taken': 'opened_long', 'price': 50000.0, 'timeframe': '1h', 'signal_strength': 0.8,
            'confidence_score': 0.75, 'indicators': {'rsi': 45.5, 'ema_20': 49800.0}, 'sentiment_data': {'sentiment_score': 0.6}, 'ml_predictions': {'price_prediction': 51000.0}, 'position_size': 0.1,
            'reasons': ['RSI oversold', 'Price above EMA'], 'volume': 1000.0, 'volatility': 0.02, 'session_id': session_id
        }
    ...
    def test_trading_sessions_logged(self, mock_strategy, mock_data_provider):
        ...
        session_id = db_manager.create_trading_session(strategy_name="TestStrategy", symbol="BTCUSDT", timeframe="1h", initial_balance=10000, strategy_config={"test": True}, session_name=session_name, mode="live")
        assert session_id > 0
        with db_manager.get_session() as session:
