...
class TestEndToEndTradingWorkflows:
    ...
    def create_test_strategy(self, strategy_type="ml_basic"):
        ...
        if strategy_type == "ml_basic":
            ...
            ensemble_gen = WeightedVotingSignalGenerator(
                generators={ml_gen: 0.6, tech_gen: 0.4}
            )
    ...
    def test_position_lifecycle_integration(self):
        ...
        for i in range(20, len(df)):
            ...
            # Entry logic
            if (decision.signal.direction == SignalDirection.BUY and 
                decision.position_size > 0 and 
                len(positions) == 0):  # No existing position
    ...
    def test_concurrent_strategy_execution(self):
        ...
        different_signals = sum(
            1 for ml_d, tech_d in zip(ml_decisions, tech_decisions)
