...
class MockDatabaseManager:
    ...
    def create_trading_session(self, strategy_name: str, symbol: str, 
                             timeframe: str, initial_balance: float,
                             strategy_config: Optional[Dict] = None,
                             mode: str = 'backtest') -> int:
    ...
    def end_trading_session(self, session_id: Optional[int] = None):
        ...
        if session_id and session_id in self._sessions:
            ...
            session_trades = [t for t in self._trades.values() 
                            if t.get('session_id') == session_id]
            session['total_trades'] = len(session_trades)
            session['winning_trades'] = len([t for t in session_trades 
                                           if t.get('pnl', 0) > 0])
    ...
    def log_trade(self, symbol: str, side: PositionSide, entry_price: float,
                  exit_price: float, size: float, entry_time: datetime,
                  exit_time: datetime, pnl: float, exit_reason: str,
                  strategy_name: str, source: TradeSource = TradeSource.BACKTEST,
                  order_id: Optional[str] = None, fees: float = 0.0,
                  slippage: float = 0.0, session_id: Optional[int] = None) -> int:
    ...
    def log_position(self, symbol: str, side: PositionSide, entry_price: float,
                     size: float, strategy_name: str, order_id: Optional[str] = None,
                     stop_loss: Optional[float] = None, take_profit: Optional[float] = None,
                     confidence_score: Optional[float] = None, quantity: Optional[float] = None,
    ...
    def update_position(self, position_id: int, current_price: Optional[float] = None,
                       stop_loss: Optional[float] = None, take_profit: Optional[float] = None,
                       status: Optional[OrderStatus] = None, notes: Optional[str] = None):
    ...
    def log_event(self, event_type: EventType, description: str, metadata: Optional[Dict] = None,
                  severity: str = 'info', session_id: Optional[int] = None) -> int:
        """Log an event"""
    ...
    def log_account_snapshot(self, balance: float, equity: float, margin_used: float = 0.0,
                           free_margin: float = 0.0, margin_level: Optional[float] = None,
    ...
    def log_strategy_execution(self, signal: str, confidence: float, indicators: Dict[str, Any],
                             market_conditions: Dict[str, Any], metadata: Optional[Dict] = None,
                             session_id: Optional[int] = None):
