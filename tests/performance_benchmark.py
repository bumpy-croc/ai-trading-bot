...
class TestPerformanceBenchmark:
    ...
    def run_test_command(self, command: List[str], name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            benchmark_result = {
                ...
                "tests_per_second": round(test_count / duration, 2) if duration > 0 and test_count > 0 else 0
            }
            
            print(f"✅ {name}: {duration:.2f}s ({test_count} tests, {benchmark_result['tests_per_second']:.1f} tests/sec)")
    ...
    def print_summary(self, benchmarks: List[Dict[str, Any]]):
        ...
        for benchmark in benchmarks:
            ...
            print(f"{status} {benchmark['name']:<20} {benchmark['duration']:>6.2f}s ({benchmark['test_count']:>3} tests)")
