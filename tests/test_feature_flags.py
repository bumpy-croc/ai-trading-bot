...
def test_overrides_dominate_repo_defaults():
    with patched_env({
        "FEATURE_FLAGS_OVERRIDES": json.dumps({"use_prediction_engine": True}),
    }):
        assert is_enabled("use_prediction_engine", default=False) is True


def test_unset_flag_returns_default():
    # Ensure absence falls back to provided default
    with patched_env({
        "FEATURE_FLAGS_OVERRIDES": json.dumps({}),
    }):
...
def test_string_flags_supported_from_env():
    with patched_env({
        "FEATURE_EXPERIMENT_BUCKET": "beta",
    }):
...
def test_resolve_all_contains_expected_keys():
    with patched_env({
        ...
        # At least contains the key from overrides or defaults
