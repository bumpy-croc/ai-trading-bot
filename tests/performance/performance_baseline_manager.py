...
class PerformanceBaselineManager:
    ...
    def record_measurement(self, test_name: str, value: float, 
    ...
    def check_performance(self, test_name: str, value: float) -> Dict[str, Any]:
        ...
        baseline = self.baselines["baselines"][test_name]
        target = baseline["target_ms"] if "ms" in test_name else baseline.get("target_mb", baseline["target_ms"])
        warning = baseline["warning_ms"] if "ms" in test_name else baseline.get("warning_mb", baseline["warning_ms"])
        critical = baseline["critical_ms"] if "ms" in test_name else baseline.get("critical_mb", baseline["critical_ms"])
    ...
    def generate_regression_report(self, analysis: Dict[str, Any]) -> str:
        ...
        if analysis["regressions_detected"]:
            ...
            for reg in analysis["regressions_detected"]:
                report.append(f"  - {reg['test_name']}: {reg['percentage']:+.1f}% "
        ...
        if analysis["warnings"]:
            ...
            for warn in analysis["warnings"]:
                report.append(f"  - {warn['test_name']}: {warn['percentage']:+.1f}% "
        ...
        if analysis["improvements"]:
            ...
            for imp in analysis["improvements"]:
                report.append(f"  - {imp['test_name']}: {imp['improvement_pct']:.1f}% faster "
