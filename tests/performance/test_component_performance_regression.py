...
class PerformanceBaseline:
    ...
    def check_performance(
        self, test_name: str, actual_value: float, metric_type: str = "ms"
    ) -> Dict[str, Any]:
        ...
        if metric_type == "ms":
            ...
            status = (
                ...
                else "acceptable" if actual_value <= max_allowed else "regression"
            )
...
class TestComponentPerformanceRegression:
    ...
    def test_signal_generation_performance(self):
        ...
        for name, generator in generators:
            ...
            # Measure performance
            for _ in range(20):
                _, exec_time = self.measure_execution_time(generator.generate_signal, df, 50)
            ...
            print(
                f"{name} - Avg: {avg_time:.2f}ms, Max: {max_time:.2f}ms, Status: {result['status']}"
            )
    ...
    def test_risk_manager_performance(self):
        ...
        signal = Signal(
            direction=SignalDirection.BUY, strength=0.8, confidence=0.9, metadata={"atr": 200.0}
        )
    ...
    def test_position_sizer_performance(self):
        ...
        signal = Signal(direction=SignalDirection.BUY, strength=0.8, confidence=0.9, metadata={})
    ...
    def test_complete_decision_cycle_performance(self):
        ...
        for i in range(25, 45):
            _, exec_time = self.measure_execution_time(strategy.process_candle, df, i, balance)
            times.append(exec_time)
        ...
        print(
            f"Complete Decision Cycle - Avg: {avg_time:.2f}ms, Max: {max_time:.2f}ms, "
        ...
        # Additional checks
        assert p95_time <= result["max_allowed"] * 1.5, f"P95 latency too high: {p95_time:.2f}ms"
    ...
    def test_batch_processing_performance(self):
        ...
        print(
            f"Batch Processing (100 candles) - Total: {total_time_ms:.2f}ms, "
...
class TestPerformanceUnderLoad:
    ...
    def test_concurrent_strategy_performance(self):
        ...
        for i, strategy in enumerate(strategies):
            ...
            thread = threading.Thread(target=process_strategy, args=(strategy, start_idx, end_idx))
        ...
        print(
            f"Concurrent Processing - {len(strategies)} strategies, "
    ...
    def test_memory_stability_under_load(self):
        ...
        print(
            f"Memory Stability - Initial: {initial_memory:.1f}MB, "
        ...
        assert max_memory <= initial_memory + 50, f"Memory spike too high: {max_memory:.1f}MB"
