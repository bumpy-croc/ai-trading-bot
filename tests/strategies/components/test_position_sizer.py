...
class TestFixedFractionSizer:
    ...
    def test_calculate_size_buy_signal_without_adjustments(self):
        ...
        sizer = FixedFractionSizer(
            fraction=0.02, adjust_for_confidence=False, adjust_for_strength=False
        )
        signal = self.create_test_signal(SignalDirection.BUY, 0.8, 0.9)
        balance = 10000.0
        risk_amount = 200.0
    ...
    def test_get_parameters(self):
        """Test get_parameters method"""
        sizer = FixedFractionSizer(
            fraction=0.03, adjust_for_confidence=False, adjust_for_strength=True
        )
        params = sizer.get_parameters()
...
class TestKellySizer:
    ...
    def test_kelly_sizer_initialization_custom(self):
        ...
        sizer = KellySizer(
            win_rate=0.6, avg_win=0.03, avg_loss=0.02, kelly_fraction=0.5, lookback_period=200
        )
    ...
    def test_calculate_size_buy_signal(self):
        ...
        sizer = KellySizer(win_rate=0.6, avg_win=0.02, avg_loss=0.015, kelly_fraction=0.5)
        signal = self.create_test_signal(SignalDirection.BUY, 0.8, 0.9)
        balance = 10000.0
        risk_amount = 2000.0
...
class TestRegimeAdaptiveSizer:
    ...
    def create_test_regime(
        self, trend="trend_up", volatility="low_vol", confidence=0.8, duration=10, strength=0.7
    ):
    ...
    def test_regime_adaptive_sizer_initialization_custom(self):
        ...
        custom_multipliers = {"bull_low_vol": 2.0, "bear_high_vol": 0.1}
        sizer = RegimeAdaptiveSizer(
            base_fraction=0.05, regime_multipliers=custom_multipliers, volatility_adjustment=False
        )
        assert sizer.base_fraction == 0.05
    ...
    def test_partial_custom_multipliers_no_keyerror(self):
        ...
        sizer = RegimeAdaptiveSizer(
            base_fraction=0.03, regime_multipliers={"bull_low_vol": 2.0}  # Only one multiplier
        )
        signal = self.create_test_signal(SignalDirection.BUY, 0.8, 0.9)
        balance = 10000.0
    ...
    def test_update_regime_multipliers(self):
        ...
        new_multipliers = {"bull_low_vol": 2.5, "bear_high_vol": 0.15}
