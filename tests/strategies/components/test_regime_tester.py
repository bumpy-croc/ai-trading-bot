...
class TestRegimeTesterVolatilityParsing:
    ...
    def test_regime_string_parsing_with_vol_suffix(self):
        ...
        test_cases = [
            ...
            ('range_medium_vol', 'range_medium', 'vol'),  # 3 parts: range, medium, vol -> legacy format
        ...
        for regime_type, expected_trend, expected_vol in test_cases:
            ...
            assert trend_str == expected_trend, f"Expected trend '{expected_trend}', got '{trend_str}' for '{regime_type}'"
            assert volatility_str == expected_vol, f"Expected vol '{expected_vol}', got '{volatility_str}' for '{regime_type}'"
    ...
    def test_regime_string_parsing_with_logging(self, mock_logger):
        ...
        if len(regime_parts) >= 4:
            ...
            mock_logger.warning(f"Unexpected regime_type format: '{regime_type}'. Using fallback values.")
            trend_str = 'range'
            volatility_str = 'low_vol'
    ...
    def test_volatility_enum_conversion_integration(self):
        ...
        for regime_type, expected_trend, expected_vol in test_cases:
            ...
            assert trend == expected_trend, f"Expected trend {expected_trend}, got {trend} for '{regime_type}'"
            assert volatility == expected_vol, f"Expected vol {expected_vol}, got {volatility} for '{regime_type}'"
    ...
    def test_bug_fix_verification(self):
        ...
        assert volatility_high == VolLabel.HIGH  # Should be HIGH, not fallback due to unrecognized "high"
