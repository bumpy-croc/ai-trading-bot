...
from src.trading.shared.sentiment import merge_historical_sentiment, apply_live_sentiment
...
def test_merge_historical_sentiment():
    ...
    assert out['sentiment_score'].isna().sum() == 2 or out['sentiment_score'].ffill().isna().sum() == 0


def test_apply_live_sentiment_adds_freshness():
    now = datetime.now()
    idx = pd.date_range(end=now, periods=10, freq='H')
    df = pd.DataFrame({'open':[1]*10,'high':[1]*10,'low':[1]*10,'close':[1]*10,'volume':[1]*10}, index=idx)
    sp = DummySentimentProvider()
