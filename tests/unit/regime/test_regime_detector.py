...
def test_regime_detector_trend_up_basic():
    ...
    cfg = RegimeConfig(slope_window=50, atr_window=14, atr_percentile_lookback=60, hysteresis_k=2, min_dwell=5)
...
def test_regime_detector_range_label_under_low_slope():
    ...
    cfg = RegimeConfig(slope_window=50, atr_window=14, atr_percentile_lookback=60, trend_threshold=0.0, hysteresis_k=1, min_dwell=1)
    rd = RegimeDetector(cfg)
    out = rd.annotate(df)
...
def test_regime_hysteresis_prevents_flips():
    ...
    df = pd.concat(parts)
    cfg = RegimeConfig(slope_window=30, atr_window=14, atr_percentile_lookback=60, hysteresis_k=5, min_dwell=30)
    rd = RegimeDetector(cfg)
    out = rd.annotate(df)
