...
class BearStrategy(BaseStrategy):
	...
	def check_short_entry_conditions(self, df: pd.DataFrame, index: int) -> bool:
		...
		return trend_down and price_below_long and macd_negative and rsi_bearish and meets_confidence
	...
	def calculate_position_size(self, df: pd.DataFrame, index: int, balance: float) -> float:
		if index >= len(df) or balance <= 0:
			return 0.0
		confidence = float(df["prediction_confidence"].iloc[index]) if "prediction_confidence" in df.columns else 0.0
		dynamic_size = self.BASE_POSITION_SIZE * max(0.0, min(1.0, confidence))
		sized_ratio = max(self.MIN_POSITION_SIZE_RATIO, min(self.MAX_POSITION_SIZE_RATIO, dynamic_size))
