...
def _log_return_normalize(
    ...
    rolling_mean = df["volume"].rolling(1000).mean()
    rolling_std = df["volume"].rolling(1000).std().replace(0, np.nan)
    df["volume_z"] = ((df["volume"] - rolling_mean) / (rolling_std + 1e-9)).fillna(0.0)
...
class ModelRegistry:
    """Singleton-like registry holding all ML models and their metadata."""
    ...
    def _register(cls, model: ModelInfo) -> None:
        assert cls._models is not None  # mypy
    ...
    def load_model(cls, name: str) -> ModelInfo:
        cls._init_registry()
        assert cls._models is not None
