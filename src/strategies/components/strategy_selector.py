...
class SelectionConfig:
    ...
    def __post_init__(self):
        ...
        total_weight = (self.sharpe_weight + self.return_weight + self.drawdown_weight + 
                       self.win_rate_weight + self.volatility_weight + self.regime_weight)
...
class StrategySelector:
    ...
    def select_best_strategy(self, available_strategies: dict[str, PerformanceTracker],
                            current_regime: Optional[RegimeContext] = None,
                            exclude_strategies: Optional[list[str]] = None) -> Optional[str]:
        """
        ...
        strategy_scores = self._calculate_strategy_scores(
            eligible_strategies, current_regime
        )
        ...
        self.logger.info(f"Selected best strategy: {best_strategy.strategy_id} "
                        f"(score: {best_strategy.total_score:.3f})")
        
        return best_strategy.strategy_id
    
    def rank_strategies(self, available_strategies: dict[str, PerformanceTracker],
                        current_regime: Optional[RegimeContext] = None,
                        exclude_strategies: Optional[list[str]] = None) -> list[StrategyScore]:
        """
    ...
    def compare_strategies(self, strategy_ids: list[str],
                          ...
                          current_regime: Optional[RegimeContext] = None) -> dict[str, Any]:
        """
        ...
        strategies_to_compare = {
            sid: tracker for sid, tracker in performance_trackers.items()
        ...
        # Calculate scores
        strategy_scores = self._calculate_strategy_scores(
            strategies_to_compare, current_regime
        )
    ...
    def get_regime_specific_ranking(self, available_strategies: dict[str, PerformanceTracker],
                                   regime_type: str) -> list[tuple[str, float]]:
        """
    ...
    def _filter_eligible_strategies(self, available_strategies: dict[str, PerformanceTracker],
                                   exclude_strategies: list[str]) -> dict[str, PerformanceTracker]:
        """Filter strategies based on minimum requirements"""
        eligible = {}
        ...
        for strategy_id, tracker in available_strategies.items():
            ...
            if tracker.trade_count < self.config.min_trades_for_consideration:
                self.logger.debug(f"Strategy {strategy_id} excluded: insufficient trades "
            ...
            if tracker.trades:
                ...
                if days_active < self.config.min_days_active:
                    self.logger.debug(f"Strategy {strategy_id} excluded: insufficient active days "
    ...
    def _calculate_strategy_scores(self, strategies: dict[str, PerformanceTracker],
                                  current_regime: Optional[RegimeContext]) -> list[StrategyScore]:
        ...
        for strategy_id, tracker in strategies.items():
            ...
            total_score = self._calculate_total_score(
                criteria_scores, regime_scores, risk_adjusted_score, 
                correlation_penalty, current_regime
            )
    ...
    def _calculate_criteria_scores(self, metrics: PerformanceMetrics,
                                  all_metrics: dict[str, PerformanceMetrics]) -> dict[SelectionCriteria, float]:
    ...
    def _calculate_regime_scores(self, tracker: PerformanceTracker,
                                current_regime: Optional[RegimeContext]) -> dict[str, float]:
    ...
    def _calculate_regime_score(self, regime_perf) -> float:
        ...
        score = (sharpe_score * 0.3 + win_rate_score * 0.25 + 
                drawdown_score * 0.25 + return_score * 0.2)
    ...
    def _calculate_correlation_penalty(self, strategy_id: str,
        ...
        if max_correlation > self.config.correlation_threshold:
            penalty += (max_correlation - self.config.correlation_threshold) * self.config.correlation_penalty_factor
    ...
    def _calculate_total_score(self, criteria_scores: dict[SelectionCriteria, float],
                              regime_scores: dict[str, float],
                              risk_adjusted_score: float,
                             correlation_penalty: float,
                             current_regime: Optional[RegimeContext]) -> float:
    ...
    def _normalize_volatility_score(self, volatility: float, all_volatilities: list[float]) -> float:
