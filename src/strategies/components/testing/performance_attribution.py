...
class PerformanceAttributionAnalyzer:
    ...
    def _validate_test_data(self) -> None:
        ...
        if len(self.test_data) < 100:
            raise ValueError(
                "Test data must have at least 100 rows for meaningful attribution analysis"
            )
    ...
    def _calculate_baseline_metrics(self) -> dict[str, float]:
        ...
        return {
            "total_return": (self.test_data["close"].iloc[-1] / self.test_data["close"].iloc[0])
            - 1,
    ...
    def analyze_strategy_attribution(
        self, strategy: Strategy, initial_balance: float = 10000.0
    ) -> AttributionReport:
        """
        ...
        interaction_effects = self._calculate_interaction_effects(strategy, baseline_results)
        ...
        if abs(baseline_results["total_return"]) < EPSILON:
            ...
            explained_performance = min(
                1.0, abs(total_attribution) / abs(baseline_results["total_return"])
            )
    ...
    def _simulate_strategy(self, strategy: Strategy, initial_balance: float) -> dict[str, Any]:
        ...
        if error_rate > ERROR_RATE_THRESHOLD:
            logger.error(
                f"Strategy simulation failed: {error_rate:.1%} error rate ({error_count}/{total_iterations} iterations)"
            )
            raise ValueError(
                f"Excessive errors in strategy simulation: {error_rate:.1%} error rate (threshold: {ERROR_RATE_THRESHOLD:.1%})"
            )
    ...
    def _analyze_signal_generator_attribution(
        ...
        consistency = (
            1 - (np.std(confidences) / (np.mean(confidences) + 0.1)) if confidences else 0.0
        )
        ...
        return ComponentAttribution(
            ...
            risk_adjusted_contribution=(
                signal_contribution / baseline_results["volatility"]
                if baseline_results["volatility"] > 0
                else 0.0
            ),
            volatility_contribution=baseline_results["volatility"]
            * 0.6,  # Estimate 60% from signals
            drawdown_contribution=baseline_results["max_drawdown"]
            * 0.5,  # Estimate 50% from signals
    ...
    def _analyze_risk_manager_attribution(
        ...
        position_consistency = (
            1 - (np.std(position_sizes) / (np.mean(position_sizes) + 0.1))
            if position_sizes
            else 0.0
        )
        ...
        return ComponentAttribution(
            ...
            volatility_contribution=baseline_results["volatility"]
            * 0.3,  # Estimate 30% from risk management
            drawdown_contribution=baseline_results["max_drawdown"]
            * 0.8,  # Estimate 80% from risk management
            component_effectiveness=effectiveness,
            component_consistency=consistency,
            component_reliability=reliability,
    ...
    def _analyze_position_sizer_attribution(
        ...
        sizing_contribution = self._estimate_sizing_contribution(
            position_sizes, trades, baseline_results
        )
        ...
        size_consistency = (
            1 - (np.std(size_fractions) / (np.mean(size_fractions) + 0.01))
            if size_fractions
            else 0.0
        )
        ...
        return ComponentAttribution(
            ...
            best_trade_contribution=(
                max([t["return"] * t["position_size"] for t in trades]) if trades else 0.0
            ),
            worst_trade_contribution=(
                min([t["return"] * t["position_size"] for t in trades]) if trades else 0.0
            ),
            risk_adjusted_contribution=(
                sizing_contribution / baseline_results["volatility"]
                if baseline_results["volatility"] > 0
                else 0.0
            ),
            volatility_contribution=baseline_results["volatility"]
            * 0.1,  # Estimate 10% from position sizing
            drawdown_contribution=baseline_results["max_drawdown"]
            * 0.2,  # Estimate 20% from position sizing
            component_effectiveness=effectiveness,
            component_consistency=consistency,
            component_reliability=reliability,
            synergy_score=0.9,  # Position sizing has high synergy with other components
            substitution_impact=sizing_contribution
            * 1.2,  # High impact if replaced with poor sizing
            regime_attribution={},  # Placeholder
            optimization_potential=1.0 - effectiveness,
            recommended_adjustments=recommendations,
        )
        ...
        for signal_data in signals:
            ...
            if corresponding_trade:
                ...
                if (
                    ...
                    signal.direction == SignalDirection.SELL and corresponding_trade["return"] < 0
                ):
                    accurate_signals += 1
    ...
    def _generate_signal_recommendations(
        ...
        if not recommendations:
            recommendations.append(
                "Signal generator performing well - consider fine-tuning parameters"
            )
    ...
    def _generate_sizing_recommendations(
        ...
        if optimality < 0.6:
            recommendations.append(
                "Improve position sizing algorithm to better match trade outcomes"
            )
        ...
        if consistency < 0.7:
            recommendations.append(
                "Improve position sizing consistency across different market conditions"
            )

        if not recommendations:
            recommendations.append(
                "Position sizing performing well - consider regime-adaptive enhancements"
            )
    ...
    def analyze_component_replacement_impact(
        self,
        strategy: Strategy,
        component_type: str,
        ...
        if internal_component_type is None:
            raise ValueError(
                f"Invalid component_type: '{component_type}'. Must be one of: {list(component_type_map.keys())}"
            )

        # Create modified strategy with replacement component
        modified_strategy = self._create_modified_strategy(
            strategy, internal_component_type, replacement_component
        )
        ...
        return {
            ...
            "return_impact_pct": (
                (return_impact / abs(original_results["total_return"]) * 100)
                if original_results["total_return"] != 0
                else 0
            ),
