...
class TestDatasetGenerator:
    ...
    def _create_extreme_volatility_case(
        self, data: pd.DataFrame, rng: np.random.Generator
    ) -> pd.DataFrame:
    ...
    def _create_extreme_outliers_case(
        self, data: pd.DataFrame, rng: np.random.Generator
    ) -> pd.DataFrame:
    ...
    def create_regime_labeled_dataset(
        self,
        scenario_name: str,
    ...
    def _generate_regime_labels(self, data: pd.DataFrame, method: str) -> pd.DataFrame:
        ...
        if method == "simple":
            ...
            regime_data["confidence"] = ma_diff.fillna(0.7).clip(
                0.5, 0.95
    ...
    def _calculate_regime_duration(self, regime_data: pd.DataFrame) -> pd.Series:
        ...
        regime_changes = (regime_data["regime_type"] != regime_data["regime_type"].shift()).fillna(
            True
        )
    ...
    def get_comprehensive_test_suite(self, seed: int | None = None) -> dict[str, pd.DataFrame]:
        ...
        for scenario in self.market_scenarios:
            test_suite[f"scenario_{scenario.name}"] = self.generate_synthetic_dataset(
                scenario.name, seed
            )
        ...
        for edge_case in edge_cases:
            test_suite[f"edge_case_{edge_case}"] = self.generate_edge_case_dataset(
                edge_case, seed=seed
            )
