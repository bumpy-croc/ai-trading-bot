...
class PerformanceComparisonEngine:
    ...
    def compare_strategies(
        ...
        try:
            ...
            if self.backtest_engine is None:
                raise ValueError("No backtest engine provided. Please provide a backtest engine to run comparisons.")
            ...
            equivalence_results = self._perform_equivalence_tests(
                legacy_results, new_results
            )
    ...
    def _run_backtest(
        ...
        self.logger.debug(f"Running backtest for {strategy_type} strategy: {strategy.__class__.__name__}")
        ...
        if 'timestamp' not in results.columns:
            if results.index.name == 'timestamp' or hasattr(results.index, 'name'):
                results = results.reset_index()
            else:
                raise ValueError(f"Backtest results missing 'timestamp' column for {strategy_type} strategy")
    ...
    def _perform_equivalence_tests(
        ...
        tost_result = self.equivalence_tests.two_one_sided_test(
            legacy_returns, new_returns
        )
    ...
    def _assess_overall_result(self, result: StrategyComparisonResult) -> None:
        ...
        if parity_result == ValidationResult.PASS:
            if not statistical_failures and equivalence_passed:
                ...
                result.certification_status = "CONDITIONAL - Performance Parity with Statistical Differences"
                result.recommendations = [
        ...
        if result.parity_report.metrics_failed > 0:
            failed_metrics = [
                comp.metric_name for comp in result.parity_report.metric_comparisons
        ...
        if result.parity_report.equity_curve_correlation < self.config.minimum_correlation_threshold:
            result.recommendations.append(
    ...
    def _export_results(self, result: StrategyComparisonResult) -> None:
        ...
        try:
            ...
            from src.strategies.components.testing.performance_parity_validator import PerformanceParityReporter
            
            # CSV export
            PerformanceParityReporter.export_to_csv(
                result.parity_report,
                str(export_dir / f"{base_filename}_parity_metrics.csv")
            )
            
            # JSON export
            PerformanceParityReporter.export_to_json(
                result.parity_report,
                str(export_dir / f"{base_filename}_parity_report.json")
            )
            ...
            if result.legacy_backtest_results is not None:
                result.legacy_backtest_results.to_csv(
                    export_dir / f"{base_filename}_legacy_backtest.csv",
                    index=False
                )
            
            if result.new_backtest_results is not None:
                result.new_backtest_results.to_csv(
                    export_dir / f"{base_filename}_new_backtest.csv",
                    index=False
                )
