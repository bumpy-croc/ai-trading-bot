...
class TestPerformanceMonitor(unittest.TestCase):
    ...
    def test_is_underperforming_true(self):
        ...
        current = PerformanceMetrics(
            ...
            volatility=0.15, sharpe_ratio=0.3, sortino_ratio=0.4, calmar_ratio=1.2,
            max_drawdown=0.25, var_95=-0.03, total_trades=30, winning_trades=12,
            losing_trades=18, win_rate=0.4, avg_win=0.025, avg_loss=-0.015,
            profit_factor=1.2, expectancy=0.002, avg_trade_duration=24.0,
            trades_per_day=1.0, hit_rate=0.4, max_drawdown_duration=5.0,
            current_drawdown=0.1, drawdown_recovery_time=2.0, best_trade=0.05,
            worst_trade=-0.04, consecutive_wins=3, consecutive_losses=4,
            period_start=datetime.now() - timedelta(days=30),
            period_end=datetime.now(), period_type=PerformancePeriod.MONTHLY
        )
        
        # Create baseline metrics with good performance
        baseline = PerformanceMetrics(
            total_return=500.0, total_return_pct=0.25, annualized_return=0.91,
            volatility=0.12, sharpe_ratio=1.5, sortino_ratio=2.0, calmar_ratio=4.5,
            max_drawdown=0.1, var_95=-0.02, total_trades=50, winning_trades=35,
            ...
            period_end=datetime.now() - timedelta(days=30), period_type=PerformancePeriod.MONTHLY
        )
    ...
    def test_is_underperforming_false(self):
        ...
        current = PerformanceMetrics(
            total_return=600.0, total_return_pct=0.30, annualized_return=1.1,
            volatility=0.10, sharpe_ratio=2.0, sortino_ratio=2.5, calmar_ratio=5.5,
            max_drawdown=0.08, var_95=-0.015, total_trades=40, winning_trades=32,
            losing_trades=8, win_rate=0.8, avg_win=0.035, avg_loss=-0.008,
            profit_factor=2.8, expectancy=0.0075, avg_trade_duration=18.0,
            trades_per_day=1.3, hit_rate=0.8, max_drawdown_duration=2.0,
            current_drawdown=0.02, drawdown_recovery_time=0.5, best_trade=0.07,
            worst_trade=-0.015, consecutive_wins=12, consecutive_losses=1,
            period_start=datetime.now() - timedelta(days=30),
            period_end=datetime.now(), period_type=PerformancePeriod.MONTHLY
        )
    ...
    def test_calculate_statistical_significance(self):
        ...
        baseline_metrics = PerformanceMetrics(
            total_return=1000.0, total_return_pct=0.5, annualized_return=1.8,
            volatility=0.08, sharpe_ratio=2.5, sortino_ratio=3.0, calmar_ratio=7.5,
            max_drawdown=0.06, var_95=-0.01, total_trades=50, winning_trades=40,
            losing_trades=10, win_rate=0.8, avg_win=0.04, avg_loss=-0.005,
            profit_factor=3.2, expectancy=0.01, avg_trade_duration=16.0,
            trades_per_day=1.5, hit_rate=0.8, max_drawdown_duration=1.5,
            current_drawdown=0.01, drawdown_recovery_time=0.3, best_trade=0.08,
            worst_trade=-0.01, consecutive_wins=15, consecutive_losses=1,
            period_start=datetime.now() - timedelta(days=90),
            period_end=datetime.now() - timedelta(days=60), period_type=PerformancePeriod.QUARTERLY
        )
    ...
    def test_determine_timeframe_severity(self):
        ...
        current = PerformanceMetrics(
            total_return=50.0, total_return_pct=0.025, annualized_return=0.09,
            volatility=0.20, sharpe_ratio=0.1, sortino_ratio=0.15, calmar_ratio=0.45,
            max_drawdown=0.35, var_95=-0.05, total_trades=25, winning_trades=8,
            losing_trades=17, win_rate=0.32, avg_win=0.02, avg_loss=-0.018,
            profit_factor=0.9, expectancy=-0.001, avg_trade_duration=30.0,
            trades_per_day=0.8, hit_rate=0.32, max_drawdown_duration=8.0,
            current_drawdown=0.2, drawdown_recovery_time=5.0, best_trade=0.04,
            worst_trade=-0.06, consecutive_wins=2, consecutive_losses=6,
            period_start=datetime.now() - timedelta(days=30),
            period_end=datetime.now(), period_type=PerformancePeriod.MONTHLY
        )
    ...
    def test_should_switch_strategy_insufficient_data(self):
        ...
        decision = self.monitor.should_switch_strategy(
            "empty_strategy", empty_tracker, market_data
        )
    ...
    @patch('src.strategies.components.performance_monitor.PerformanceMonitor._analyze_multiple_timeframes')
    ...
    def test_get_degradation_history(self):
        ...
        self.monitor.degradation_history.extend([
            ...
            (datetime.now() - timedelta(days=40), "strategy1", DegradationSeverity.MINOR)  # Too old
        ])
