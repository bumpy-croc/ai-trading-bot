...
class Strategy:
    ...
    def should_exit_position(
        self, position: Position, current_data: MarketData, regime: Optional[RegimeContext] = None
    ) -> bool:
        """
    ...
    def get_stop_loss_price(
        self, entry_price: float, signal: Signal, regime: Optional[RegimeContext] = None
    ) -> float:
        """
    ...
    def _generate_signal(
        self, df: pd.DataFrame, index: int, regime: Optional[RegimeContext]
    ) -> Signal:
    ...
    def _calculate_risk_position_size(
        self, signal: Signal, balance: float, regime: Optional[RegimeContext]
    ) -> float:
    ...
    def _calculate_final_position_size(
        self, signal: Signal, balance: float, risk_amount: float, regime: Optional[RegimeContext]
    ) -> float:
    ...
    def _validate_position_size(
        self, position_size: float, signal: Signal, balance: float, regime: Optional[RegimeContext]
    ) -> float:
