...
class PerformanceMonitoringSystem:
    ...
    def update_monitoring(self, market_data: pd.DataFrame,
        ...
        try:
            ...
            if (self.current_strategy_id and 
                not self.emergency_controls.emergency_stop_active and
                self.strategy_activation_callback):
                ...
                if current_tracker:
                    ...
                    if switch_request:
                        results['switch_recommendations'].append({
                            ...
                            'confidence': switch_request.switch_decision.confidence if switch_request.switch_decision else 0.0
                        })
                        ...
                        if switch_record.status.value == 'completed':
                            ...
                            self.logger.info(f"Automatic strategy switch completed: "
            ...
            results['active_alerts'] = [alert.to_dict() for alert in self.emergency_controls.get_active_alerts()]
            results['pending_approvals'] = [req.to_dict() for req in self.emergency_controls.get_pending_approvals()]
    ...
    def request_manual_switch(self, to_strategy: str, reason: str,
                            requested_by: str, bypass_approval: bool = False) -> str:
        """
    ...
    def get_strategy_rankings(self, current_regime: Optional[RegimeContext] = None) -> list:
        ...
        """
        return self.strategy_selector.rank_strategies(
            self.available_strategies, current_regime
        )
    ...
    def get_comprehensive_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            'active_alerts': [alert.to_dict() for alert in self.emergency_controls.get_active_alerts()],
            'pending_approvals': [req.to_dict() for req in self.emergency_controls.get_pending_approvals()],
    ...
    def add_switch_callback(self, pre_switch: Optional[Callable] = None,
                          post_switch: Optional[Callable] = None) -> None:
