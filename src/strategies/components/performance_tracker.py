...
class PerformanceTracker:
    ...
    def __init__(self, strategy_id: str, max_history: int = 10000,
    ...
    def record_trade(self, trade: TradeResult) -> None:
        ...
        self.logger.debug(f"Recorded trade: PnL={trade.pnl:.2f}, Balance={self.current_balance:.2f}")
    
    def get_performance_metrics(self, period: PerformancePeriod = PerformancePeriod.ALL_TIME,
        ...
        if not filtered_trades:
            ...
            return PerformanceMetrics(
                ...
                volatility=0.0, sharpe_ratio=0.0, sortino_ratio=0.0, calmar_ratio=0.0,
                max_drawdown=0.0, var_95=0.0, total_trades=0, winning_trades=0,
                losing_trades=0, win_rate=0.0, avg_win=0.0, avg_loss=0.0,
                profit_factor=0.0, expectancy=0.0, avg_trade_duration=0.0,
                trades_per_day=0.0, hit_rate=0.0, max_drawdown_duration=0.0,
                current_drawdown=0.0, drawdown_recovery_time=0.0, best_trade=0.0,
                worst_trade=0.0, consecutive_wins=0, consecutive_losses=0,
                period_start=now, period_end=now, period_type=period
            )
    ...
    def compare_performance(self, other_tracker: 'PerformanceTracker',
        ...
        comparison = {
            'strategy_1': {
                'id': self.strategy_id,
                'metrics': self_metrics.to_dict()
            },
            'strategy_2': {
                'id': other_tracker.strategy_id,
                'metrics': other_metrics.to_dict()
            },
            'comparison': {
    ...
    def _update_running_stats(self, trade: TradeResult) -> None:
        ...
        if trade.pnl > 0:
            ...
            self.running_stats['max_win_streak'] = max(
                self.running_stats['max_win_streak'],
                self.running_stats['consecutive_wins']
            )
            ...
            if self.running_stats['current_streak_type'] == 'loss':
                self.running_stats['current_streak_count'] += 1
                self.running_stats['consecutive_losses'] = self.running_stats['current_streak_count']
            else:
            ...
            self.running_stats['max_loss_streak'] = max(
                self.running_stats['max_loss_streak'],
                self.running_stats['consecutive_losses']
            )
        
        # Update best/worst trades
        self.running_stats['best_trade'] = max(self.running_stats['best_trade'], trade.pnl_percent)
        self.running_stats['worst_trade'] = min(self.running_stats['worst_trade'], trade.pnl_percent)
    ...
    def _calculate_metrics(self, trades: list[TradeResult], period: PerformancePeriod) -> PerformanceMetrics:
        ...
        # Trade statistics
        win_rate = len(winning_trades) / len(trades) if trades else 0.0
        avg_win = statistics.mean([t.pnl_percent for t in winning_trades]) if winning_trades else 0.0
