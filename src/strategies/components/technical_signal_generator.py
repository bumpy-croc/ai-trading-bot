...
class TechnicalSignalGenerator(SignalGenerator):
    ...
    def generate_signal(
        ...
        strength = self._calculate_signal_strength(rsi_signal, macd_signal, ma_signal, bb_signal)
        ...
        return Signal(
            direction=combined_signal, strength=strength, confidence=confidence, metadata=metadata
        )
    ...
    def _ensure_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not all(col in df_copy.columns for col in macd_columns):
            df_copy = calculate_macd(df_copy, self.macd_fast, self.macd_slow, self.macd_signal)
    ...
    def _combine_signals(
        ...
        weights = {"rsi": 0.2, "macd": 0.3, "ma": 0.3, "bb": 0.2}
...
class RSISignalGenerator(SignalGenerator):
    ...
    def get_parameters(self) -> dict[str, Any]:
        ...
        params = super().get_parameters()
        params.update(
            {"period": self.period, "overbought": self.overbought, "oversold": self.oversold}
        )
        return params
    ...
    def generate_signal(
        ...
        if index < max(1, self.min_periods):
            return Signal(
                ...
                strength=0.0,
                confidence=0.0,
                metadata={"generator": self.name, "reason": "insufficient_history", "index": index},
            )
