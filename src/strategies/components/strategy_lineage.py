...
class StrategyLineageTracker:
    ...
    def register_strategy(self, strategy_id: str, parent_id: Optional[str] = None,
        ...
        if self.storage_backend:
            try:
                self.storage_backend.save_strategy_lineage(strategy_id, self.strategies[strategy_id])
    ...
    def create_branch(self, parent_strategy_id: str, branch_name: str,
        ...
        self.logger.info(f"Created branch {branch_name} ({branch_id}) from strategy {parent_strategy_id}")
        return branch_id
        ...
        for i, strategy_id in enumerate(path):
            ...
            if i > 0:
                ...
                if edge_key in self.graph_edges:
                    ...
                    strategy_info['relationship'] = edge_data.get('relationship_type', RelationshipType.PARENT).value
    ...
    def analyze_change_impact(self, strategy_id: str, change_id: str) -> Dict[str, Any]:
        ...
        for desc in descendants:
            ...
            related_changes = [
                c for c in desc_changes
                if c.created_at > change_record.created_at and
                any(comp in change_record.changed_components for comp in c.changed_components)
            ]
            
            if related_changes:
                descendant_impact.append({
                    'strategy_id': desc_id,
                    'generation_distance': desc['generation'] - self.strategies[strategy_id]['generation'],
        ...
        return {
            ...
            'propagation_depth': max([d['generation_distance'] for d in descendant_impact]) if descendant_impact else 0,
    ...
    def get_evolution_metrics(self) -> EvolutionMetrics:
        ...
        performance_improvement_rate = (
            ...
            if performance_improvements else 0.0
        )
