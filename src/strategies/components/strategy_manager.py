...
class ComponentStrategyManager:
    ...
    def execute_strategy(
        ...
        try:
            ...
            self.logger.info(
                f"Strategy executed: {signal.direction.value} signal with size {position_size:.2f}"
            )
    ...
    def _filter_risk_kwargs(self, method: Any, context: dict[str, Any]) -> dict[str, Any]:
    ...
    def get_version_performance(
        self, version_id: str, lookback_executions: int = 100
    ) -> dict[str, float]:
        """
        ...
        version_executions = [
            exec
            for exec in self.execution_history[-lookback_executions:]
        ...
        avg_execution_time = (
            sum(exec.execution_time_ms for exec in version_executions) / total_executions
        )
    ...
    def compare_versions(
        self, version_ids: list[str], metric: str = "avg_signal_confidence"
    ) -> dict[str, float]:
        """
    ...
    def get_execution_statistics(self, lookback_hours: int = 24) -> dict[str, Any]:
        ...
        avg_execution_time = (
            sum(exec.execution_time_ms for exec in recent_executions) / total_executions
        )
