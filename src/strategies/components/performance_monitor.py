...
class SwitchDecision:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            'degradation_severity': self.degradation_severity.value,
            'timeframe_results': [r.to_dict() for r in self.timeframe_results] if self.timeframe_results else None
        }
...
class PerformanceMonitor:
    ...
    def __init__(self, config: Optional[PerformanceDegradationConfig] = None):
        ...
        self.performance_baselines: dict[str, dict[TimeFrame, PerformanceMetrics]] = defaultdict(dict)
        
        # Regime-specific performance tracking
        self.regime_performance_history: dict[str, dict[str, list[PerformanceMetrics]]] = defaultdict(lambda: defaultdict(list))
    ...
    def should_switch_strategy(self, current_strategy_id: str,
                             performance_tracker: PerformanceTracker,
                             market_data: pd.DataFrame,
                             current_regime: Optional[RegimeContext] = None) -> SwitchDecision:
        """
    ...
    def update_performance_baseline(self, strategy_id: str,
                                  performance_tracker: PerformanceTracker,
    ...
    def get_regime_performance_confidence(self, strategy_id: str,
                                        current_regime: str,
                                        performance_tracker: PerformanceTracker) -> float:
        """
    ...
    def _meets_minimum_requirements(self, performance_tracker: PerformanceTracker) -> bool:
        ...
        if performance_tracker.trade_count < self.config.min_trades_for_evaluation:
            self.logger.debug(f"Insufficient trades: {performance_tracker.trade_count} < {self.config.min_trades_for_evaluation}")
            return False
        ...
        if days_active < self.config.min_days_for_evaluation:
            self.logger.debug(f"Insufficient time period: {days_active} < {self.config.min_days_for_evaluation}")
            return False
        
        return True
    
    def _analyze_multiple_timeframes(self, strategy_id: str,
                                   performance_tracker: PerformanceTracker,
                                   current_regime: Optional[RegimeContext]) -> list[TimeFrameAnalysis]:
    ...
    def _is_underperforming(self, current: PerformanceMetrics, baseline: PerformanceMetrics) -> bool:
    ...
    def _calculate_statistical_significance(self, current: PerformanceMetrics,
    ...
    def _calculate_confidence_interval(self, metrics: PerformanceMetrics,
                                     performance_tracker: PerformanceTracker,
    ...
    def _determine_timeframe_severity(self, current: PerformanceMetrics,
                                    baseline: PerformanceMetrics) -> DegradationSeverity:
        """Determine degradation severity for a timeframe"""
        # Calculate degradation scores
        sharpe_degradation = max(0, (baseline.sharpe_ratio - current.sharpe_ratio) / max(0.1, baseline.sharpe_ratio))
        drawdown_increase = max(0, (current.max_drawdown - baseline.max_drawdown) / max(0.01, baseline.max_drawdown))
        return_degradation = max(0, (baseline.total_return_pct - current.total_return_pct) / max(0.01, abs(baseline.total_return_pct)))
        
        # Weighted severity score
        severity_score = (sharpe_degradation * 0.4 + drawdown_increase * 0.3 + return_degradation * 0.3)
    ...
    def _is_regime_transition_period(self, market_data: pd.DataFrame,
                                   current_regime: Optional[RegimeContext]) -> bool:
    ...
    def _calculate_degradation_severity(self, timeframe_results: list[TimeFrameAnalysis]) -> DegradationSeverity:
    ...
    def _calculate_switch_confidence(self, timeframe_results: list[TimeFrameAnalysis],
                                   severity: DegradationSeverity) -> float:
    ...
    def get_degradation_history(self, strategy_id: Optional[str] = None,
