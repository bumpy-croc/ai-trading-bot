...
class EmergencyControls:
    ...
    def __init__(self, strategy_switcher: StrategySwitcher,
    ...
    def check_emergency_conditions(self, strategy_id: str,
                                 performance_tracker: PerformanceTracker,
                                 current_regime: Optional[RegimeContext] = None) -> EmergencyLevel:
        """
        ...
        if current_level > self.emergency_level:
            ...
            if current_level in [EmergencyLevel.HIGH, EmergencyLevel.CRITICAL]:
                self.activate_conservative_mode(f"Auto-activated due to {current_level.value} emergency")
        
        return current_level
    
    def request_manual_strategy_switch(self, from_strategy: str, to_strategy: str,
                                     reason: str, requested_by: str,
                                     bypass_approval: bool = False) -> str:
        """
        ...
        if (self.config.require_approval_for_high_risk and 
    ...
    def approve_request(self, request_id: str, approved_by: str,
        ...
        if rejection_reason:
            ...
            self.logger.info(f"Request rejected by {approved_by}: {request_id} - {rejection_reason}")
    ...
    def get_system_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            'last_emergency_check': self.last_emergency_check.isoformat() if self.last_emergency_check else None,
    ...
    def _trigger_alert(self, alert_type: AlertType, level: EmergencyLevel,
                      strategy_id: str, message: str) -> Optional[str]:
    ...
    def update_monitoring(self, strategy_performance: dict[str, PerformanceTracker],
                         current_regime: Optional[RegimeContext] = None) -> None:
        """
        ...
        if (self.last_performance_check is None or
            now - self.last_performance_check >= timedelta(minutes=self.config.performance_check_interval_minutes)):
        ...
        if (self.last_emergency_check is None or
            now - self.last_emergency_check >= timedelta(minutes=self.config.emergency_check_interval_minutes)):
