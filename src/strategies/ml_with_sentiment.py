...
class MlWithSentiment(BaseStrategy):
        ...
        if os.path.exists(metadata_path):
            try:
                ...
                if "feature_names" in self.metadata:
                    ...
                    self.sentiment_features = [
                        f for f in self.feature_names if f.startswith("sentiment_")
                    ]
    ...
    def _initialize_sentiment_provider(self, csv_path=None):
        ...
        try:
            ...
            print(
                f"Initialized sentiment provider with {len(self.sentiment_provider.data)} records"
            )
        ...
        for feature in price_features:
            if feature in df.columns:
                ...
                df[f"{feature}_normalized"] = (
                    ...
                    .apply(
                        lambda x: (
                            (x[-1] - np.min(x)) / (np.max(x) - np.min(x))
            ...
            if is_recent_data and hasattr(self.sentiment_provider, "get_live_sentiment"):
                ...
                try:
                    ...
                    if live_sentiment:
                        ...
                        sentiment_data_available = True
                        print(
                            f"✅ Applied live sentiment to {recent_mask.sum()} recent data points"
                        )
                ...
                try:
                    ...
                    sentiment_df = self.sentiment_provider.get_historical_sentiment(
                        symbol=self.trading_pair, start=start_date, end=end_date
                    )
                    ...
                    if not sentiment_df.empty:
                        ...
                        sentiment_data_available = True
                        print(
                            f"✅ Applied historical sentiment for {len(sentiment_df)} data points"
                        )
                    else:
    ...
    def _ensure_sentiment_features(self, df: pd.DataFrame, data_available: bool):
        ...
        if data_available:
            ...
            if "sentiment_primary" in df.columns:
                ...
                if recent_mask.any():
                    df.loc[recent_mask, "sentiment_confidence"] = (
                        0.9  # Higher confidence for recent data
                    )
        ...
        try:
            ...
            for i in range(self.sequence_length, len(df)):
                try:
                        ...
                        if input_window.shape[2] < self.expected_features:
                            ...
                            padding = np.zeros(
                                (
                                    ...
                                    self.expected_features - input_window.shape[2],
                                ),
                                dtype=np.float32,
                            )
                    ...
                    df.iloc[i, df.columns.get_loc("prediction_confidence")] = max(
                        0, min(1, confidence)
                    )
            ...
            if predictions_generated > 0 or prediction_errors > 0:
                print(
                    f"Generated {predictions_generated} ML predictions ({prediction_errors} errors)"
                )
    ...
    def check_entry_conditions(self, df: pd.DataFrame, index: int) -> bool:
        ...
        entry_signal = (
            predicted_return > price_increase_threshold and confidence > confidence_threshold
        )
        ...
        if entry_signal and sentiment_freshness > 0:
            ...
            print(
                f"   Sentiment primary: {df.get('sentiment_primary', pd.Series([0])).iloc[index]:.3f}"
            )
    ...
    def calculate_position_size(self, df: pd.DataFrame, index: int, balance: float) -> float:
        ...
        if "prediction_confidence" in df.columns and not pd.isna(
            df["prediction_confidence"].iloc[index]
        ):
