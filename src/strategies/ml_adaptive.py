...
class MlAdaptive(BaseStrategy):
    ...
    def __init__(
        self, name="MlAdaptive", model_path="src/ml/btcusdt_price.onnx", sequence_length=120
    ):
        super().__init__(name)
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["bear_market"] = np.where(
            (df["ma_50"] < df["ma_200"]) & (df["trend_strength"] < self.bear_trend_threshold), 1, 0
        )
    ...
    def _detect_market_regime(self, df: pd.DataFrame) -> pd.Series:
        ...
        for i in range(len(df)):
            ...
            volatility = (
                df["volatility_20"].iloc[i] if not pd.isna(df["volatility_20"].iloc[i]) else 0
            )
            ...
            # Recovery phase directly after crisis (within 24h)
                ...
                self.last_crisis_time
                and (df.index[i] - self.last_crisis_time).total_seconds() < self.SECONDS_PER_DAY
            ):
    ...
    def check_entry_conditions(self, df: pd.DataFrame, index: int) -> bool:
        ...
        if (
            current_date in self.daily_losses
            and self.daily_losses[current_date] <= -self.max_daily_loss_pct
        ):
            self.log_execution(
                signal_type="entry",
        ...
        volatility = (
            df["volatility_20"].iloc[index]
            if not pd.isna(df["volatility_20"].iloc[index])
            else 0.02
        )
        ...
        entry_conditions = [
            ...
            regime not in ["crisis", "bear"]
            or confidence >= min_confidence * 2,  # Extra caution in crisis/bear
            rsi < 70,  # Not overbought
            (trend_direction > 0)
            or rebound
            or regime in ["bull", "recovery"],  # Positive trend or rebound/bull
        ]
    ...
    def calculate_position_size(self, df: pd.DataFrame, index: int, balance: float) -> float:
        ...
        confidence = (
            ...
            if not pd.isna(df["prediction_confidence"].iloc[index])
            else 0.0
        )
        ...
        if self.consecutive_losses > 0:
            position_size *= (
                1 - self.LOSS_REDUCTION_FACTOR * self.consecutive_losses
