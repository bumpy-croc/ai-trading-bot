...
class CorrelationEngine:
    ...
    def should_update(self, now: datetime | None = None) -> bool:
        ...
        return now - self._last_update_at >= timedelta(
            hours=self.config.correlation_update_frequency_hours
        )
    ...
    def compute_size_reduction_factor(
        ...
        max_allowed = float(
            ...
            if max_exposure_override is not None
            else self.config.max_correlated_exposure
        )
        factor = 1.0
