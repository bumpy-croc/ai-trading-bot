    ...
    def _close_position(self, position: Position, reason: str):
        ...
        try:
            ...
            if self.trading_session_id is not None:
                ...
                logger.warning(
                    "⚠️ Cannot update balance in database - no trading session ID available"
                )
            ...
            if self.trading_session_id is not None:
                ...
                try:
                    _trade_db_id = self.db_manager.log_trade(
                        symbol=position.symbol,
            ...
            if position.order_id in self.position_db_ids:
                ...
                if position_db_id is not None:
                    ...
                    logger.debug(
                        f"Position {position.order_id} was not logged to database (no session ID)"
                    )
            ...
            logger.info(
                f"🏁 Closed {position.side.value} position: {position.symbol} @ ${exit_price:.2f} ({reason}) - PnL: {pnl_str}"
            )
