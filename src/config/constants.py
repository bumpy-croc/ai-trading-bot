...
DEFAULT_ACCOUNT_SNAPSHOT_INTERVAL = 3600  # Account snapshot interval (1 hour) — matches runner.py's --snapshot-interval default, the only value actually deployed anywhere (#1184)
...
DEFAULT_LOW_DIVERSIFICATION_EXPOSURE_THRESHOLD = (
    0.15  # 15% exposure triggers diversification guardrail
)
DEFAULT_LOW_DIVERSIFICATION_SIZE_FACTOR = (
    0.8  # 20% trim nudges diversification without halting trading
)
...
DEFAULT_LARGE_SINGLE_POSITION_STOP_MULTIPLIER = (
    1.2  # Widen stops to reduce churn on oversized trades
)
...
DEFAULT_NORMALIZATION_EPSILON = (
    1e-8  # Epsilon for feature normalization to prevent division by zero
)
