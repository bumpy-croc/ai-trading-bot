...
def extract_sentiment_data(df: pd.DataFrame, index: int) -> Dict:
    ...
    sentiment_columns = [
        ...
        'sentiment_volatility', 'sentiment_confidence', 'sentiment_freshness'
    ]
...
def compute_performance_metrics(
    ...
    if balance_history is not None and not balance_history.empty:
        ...
        if not daily_balance.empty and daily_balance.shape[0] >= 2 and daily_balance.pct_change().dropna().std() != 0:
