...
class LiveSentimentDemo:
    ...
    def setup_demo(self):
        ...
        try:
            ...
            print(
                f"✅ Sentiment provider initialized with {len(self.sentiment_provider.data)} historical records"
            )
    ...
    def create_demo_data(self):
        ...
        print(
            f"   Price range: ${self.demo_data['close'].min():.2f} - ${self.demo_data['close'].max():.2f}"
        )
    ...
    def demonstrate_sentiment_comparison(self):
        ...
        historical_sentiment = self.sentiment_provider.get_sentiment_for_date(
            yesterday, use_live=False
        )

        print("📚 Historical Sentiment (24h ago):")
    ...
    def simulate_live_trading_decisions(self):
        ...
        for idx, (timestamp, row) in enumerate(recent_data.iterrows()):
            ...
            if sentiment_freshness > 0:
                print("   🔴 LIVE SENTIMENT:")
            elif sentiment_freshness == 0:
                print("   📚 HISTORICAL SENTIMENT:")
            else:
            ...
            if idx >= 1:  # Need at least 2 points for signal
                entry_signal = self.strategy.check_entry_conditions(
                    processed_data, len(processed_data) - 5 + idx
                )
                if entry_signal:
                    print("   🚀 ENTRY SIGNAL DETECTED!")
                else:
                    print("   ⏸️  No entry signal")
    ...
    def demonstrate_api_freshness(self):
        ...
        for i in range(3):
            ...
            try:
                ...
                # Show cache status
                cache_age = (
                    datetime.now() - self.sentiment_provider._last_api_call
                ).total_seconds()
    ...
    def show_trading_advantages(self):
        ...
        print("   • API calls every 15 minutes (configurable)")
        print("   • Intelligent caching to avoid rate limits")
        print("   • Fallback to historical data if API fails")
        print("   • Sentiment freshness scoring")
        print("   • Confidence boosting for fresh sentiment")
