...
- **What would raise confidence, if pursued later**: (a) more out-of-sample folds as they become available (this model's usable OOS window grows by exactly the number of days that pass, one day at a time — Segment B-style live-matched windows will accumulate this way); (b) a fold or two from a period with clearer bear-market character, to directly test point 1's confound rather than only inferring it from a different study; (c) the go-forward observability the parity investigation recommended — a durable `system_event` on every guard rejection — **has already shipped** (PR #1016, merged to `develop` while this session was in flight; every rejection now writes a `system_events` row with balance state, signal strength/confidence, and open-position snapshot). A future version of this note can replace segment A's forensics-only treatment with an actual reconstructed counterfactual once enough guard-rejection history accumulates under the native (non-substitute) model — no further build needed to get there, just time.

Tracked via GH #1020 (proposal review) and #990 (closed — mechanism question answered).
