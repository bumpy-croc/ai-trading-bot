...
|---|---|---|---|---|
| `FEATURE_LIVE_PARTIAL_OPERATIONS` | `live_partial_operations` | `false` | boolean | **#734 (dev-only).** Re-enables partial exits / scale-ins in `LiveTradingEngine`. They are hard-disabled by default because the live engine currently executes them as **bookkeeping only** — no exchange order is placed, and the unit math is wrong (fraction-of-original applied to fraction-of-balance state) — which desyncs tracked size from real holdings, books phantom PnL, and frees risk budget that is still deployed. Enable only while developing the #734 fix; **never** for live capital. |
...
| `FEATURE_ENABLE_ETF_FLOW_GATE` | `enable_etf_flow_gate` | `false` | boolean | **#803.** Wraps `ml_basic`/`ml_adaptive` signal generation with the ETF net-flow gate, which vetoes NEW LONG entries while the 5-day US-spot-ETF net-flow z-score is below `DEFAULT_ETF_FLOW_LONG_BLOCK_ZSCORE` (−1.0). Resolved once at strategy build. Flows come from `ETFFlowProvider` (cache→seed fallback). Ships OFF. |
| `FEATURE_ENABLE_SENTIMENT_EXTREME_OVERLAY` | `enable_sentiment_extreme_overlay` | `false` | boolean | **#804.** Wraps `ml_sentiment` signal generation with the sentiment-extreme overlay: F&G < 15 blocks new shorts and restricts longs to a band around `DEFAULT_SENTIMENT_SUPPORT_LEVEL` (a config *parameter*, default None = no band); F&G > 70 + downtrend permits fade shorts. Composes with the flow gate (most-restrictive-wins). F&G from `FearGreedProvider` (neutral offline → inert). Ships OFF. |
| `FEATURE_ENABLE_EXPOSURE_GOVERNOR` | `enable_exposure_governor` | `false` | boolean | **#802.** Caps total gross open exposure per market regime (`ExposureGovernor`), applied in both engines' entry paths after sizing/dynamic-risk. Bear regimes cap tightest (15–20%); unknown/None regime uses the 15% conservative cap. Caps are config (`DEFAULT_EXPOSURE_CAPS`). Requires `enable_regime_detection` for non-conservative caps; otherwise every entry uses the conservative cap. Ships OFF. |
| `FEATURE_ACCOUNT_CIRCUIT_BREAKERS` | `account_circuit_breakers` | `off` | `off` / `dry_run` / `active` | **#807.** Account-level hard halts (`AccountCircuitBreaker`): daily-loss halt (2.5% of the UTC-day baseline, latched) + drawdown halt (15% peak-to-trough, clears within 5% of peak). `off` = inert; `dry_run` = evaluate + log "would halt", no action; `active` = block new entries via the pre-order gate. Read via `get_flag` (string). Force-flatten + DB baseline persistence are human-sign-off follow-ups. Ships `off`. |
| `FEATURE_ENABLE_MACRO_EVENT_GUARD` | `enable_macro_event_guard` | `false` | boolean | **#806.** Around FOMC/CPI events (from `config/macro_events.json`) blocks new entries and halves regime exposure caps, from N hours before to M hours after (per-event config). Applied via the shared pre-order gate in both engines + the legacy short path. Stale/empty calendar = no windows (fail-safe). Ships OFF. |
| `FEATURE_ENABLE_VOL_TARGET_SIZING` | `enable_vol_target_sizing` | `false` | boolean | **#805.** Wraps `ml_basic`/`ml_adaptive` position sizing with `VolatilityTargetSizer`, scaling size inversely to the regime detector's ATR percentile (constant dollar-vol per position). Bounded multiplier (`DEFAULT_VOL_TARGET_*`); pass-through when ATR percentile is unavailable. Requires `enable_regime_detection`. Ships OFF. |
