...
def upgrade() -> None:
    ...
    if not _index_exists("idx_pred_cache_expires"):
        op.execute(
            "CREATE INDEX IF NOT EXISTS idx_pred_cache_expires ON prediction_cache (expires_at)"
        )
    if not _index_exists("idx_pred_cache_model_config"):
        op.execute(
            "CREATE INDEX IF NOT EXISTS idx_pred_cache_model_config ON prediction_cache (model_name, config_hash)"
        )
    if not _index_exists("idx_pred_cache_access"):
        op.execute(
            "CREATE INDEX IF NOT EXISTS idx_pred_cache_access ON prediction_cache (last_accessed)"
        )
    if not _index_exists("idx_pred_cache_model"):
        op.execute(
            "CREATE INDEX IF NOT EXISTS idx_pred_cache_model ON prediction_cache (model_name)"
        )
    if not _index_exists("idx_pred_cache_features"):
        op.execute(
            "CREATE INDEX IF NOT EXISTS idx_pred_cache_features ON prediction_cache (features_hash)"
        )
